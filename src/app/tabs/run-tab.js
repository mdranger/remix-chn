    ...
    modalDialogCustom.prompt(null, 'Transactions will be saved in a file under ' + path, 'scenario.json', input => {
      ...
      if (fileProvider) {
        ...
        helper.createNonClashingName(newFile, fileProvider, (error, newFile) => {
          if (error) return modalDialogCustom.alert('文件创建错误. ' + newFile + ' ' + error)
          if (!fileProvider.set(newFile, txJSON)) {
            modalDialogCustom.alert('文件创建错误 ' + newFile)
          } else {
...
function contractDropdown (events, self) {
  ...
  var compFails = yo`<i title="合约编译错误！请在编译界面查看更多信息。" class="fa fa-times-circle ${css.errorIcon}" ></i>`
    ...
    <div class="${css.container}">
      ...
      <div>
        ...
        <div class="${css.button} ${css.atAddressSect}">
          <div class="${css.atAddress}" onclick=${function () { loadFromAddress() }}>合约地址</div>
  ...
  function setInputParamsPlaceHolder () {
    ...
    if (selectContractNames.selectedIndex >= 0 && selectContractNames.children.length > 0) {
      ...
      }, txHelper.inputParametersDeclarationToString(ctrabi.inputs), '部署合约', ctrEVMbc)
      self._view.createPanel.appendChild(createConstructorInstance.render())
      return
    } else {
      self._view.createPanel.innerHTML = '合约未编译'
    }
  }
  ...
  function createInstance (args, compiler) {
    ...
    if (selectedContract.contract.object.evm.bytecode.object.length === 0) {
      modalDialogCustom.alert('合约中有未完成的函数，无法部署。')
      return
    }
    ...
    if (selectedContract.contract.object.evm.deployedBytecode && selectedContract.contract.object.evm.deployedBytecode.object.length / 2 > 24576) {
      modalDialog('合约代码体积大于上限', yo`<div>Contract creation initialization returns data with length of more than 24576 bytes. The deployment will likely fails. <br>
...
function settings (container, self) {
  ...
  // Remove two options and only allow the local JS VM to be selected
    ...
    <div class="${css.crow}">
      ...
      <div class=${css.environment}>
        ...
        <select id="selectExEnvOptions" onchange=${() => { updateNetwork() }} class="${css.select}">
          <option id="vm-mode"
            title="合约执行环境为本地虚拟机内存，并不连接到区块链节点"
  ...
  var accountEl = yo`
    <div class="${css.crow}">
      <div class="${css.col1_1}">
        帐号
        <i class="fa fa-plus-circle ${css.icon}" aria-hidden="true" onclick=${newAccount} title="产生新帐号"></i>
      </div>
      <div class=${css.account}>
        <select name="txorigin" class="${css.select}" id="txorigin"></select>
        ${copyToClipboard(() => document.querySelector('#runTabView #txorigin').value)}
        <i class="fa fa-pencil-square-o ${css.icon}" aria-hiden="true" onclick=${signMessage} title="使用本帐号进行签名"></i>
      </div>
    </div>
  `
  var gasPriceEl = yo`
    <div class="${css.crow}">
      <div class="${css.col1_1}">Gas 上限</div>
      <input type="number" class="${css.col2}" id="gasLimit" value="3000000">
    </div>
  `
  var valueEl = yo`
    <div class="${css.crow}">
      <div class="${css.col1_1}">Gas 数值</div>
      <input type="text" class="${css.col2_1}" id="value" value="0" title="请输入Gas数值并选择单位">
  ...
  function signMessage (event) {
    self._deps.udapp.getAccounts((err, accounts) => {
      if (err) { addTooltip(`未发现帐号信息: ${err}`) }
      var signMessageDialog = { 'title': '对信息签名', 'text': '请输入要签名的信息', 'inputvalue': '输入信息' }
      var $txOrigin = container.querySelector('#txorigin')
      ...
      if (isVM) {
        ...
        modalDialogCustom.promptPassphrase('签名帐号的密码', '请输入帐号密码以用于签名', '', (passphrase) => {
          modalDialogCustom.promptMulti(signMessageDialog, (message) => {
            const hashedMsg = executionContext.web3().sha3(message)
            try {
