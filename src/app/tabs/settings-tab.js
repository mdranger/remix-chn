...
module.exports = class SettingsTab {
  ...
  render () {
    ...
    self._view.config.general = yo`
      <div class="${css.info}">
          <div class=${css.title}>常用设置</div>
          <div class="${css.crow}">
            <div>${self._view.generateContractMetadata}</div>
            <span class="${css.checkboxText}">产生智能合约的辅助信息，包括在合约文件的目录产生一个JSON文件。允许指定合约所需要的库函数地址。如果没有指定库函数地址，Remix会自动部署相应合约。</span>
          </div>
          <div class="${css.crow}">
            <div>${self._view.optionVM}</div>
            <span class="${css.checkboxText}">启动时自动使用 Ethereum 虚拟机</span>
          </div>
          <div class="${css.crow}">
            <div><input id="editorWrap" type="checkbox" onchange=${function () { self._deps.editor.resize(this.checked) }}></div>
            <span class="${css.checkboxText}">文本换行</span>
          </div>
          <div class="${css.crow}">
    ...
    self._view.gistToken = yo`
      <div class="${css.info}">
        <div class=${css.title}>版本控制功能</div>
    ...
    self._view.config.themes = yo`
      <div class="${css.info}">
        <div class=${css.title}>界面风格</div>
        <div class=${css.attention}>
          <i title="Select the theme" class="${css.icon} fa fa-exclamation-triangle" aria-hidden="true"></i>
          <span>切换界面风格需要重新加载页面</span>
        </div>
        <div class="${css.crow}">
          ${self._view.theme.light}
          <label for="themeLight">浅色界面</label>
        </div>
        <div class="${css.crow}">
          ${self._view.theme.dark}
          <label for="themeDark">深色界面</label>
        </div>
      </div>`
    self._view.config.plugins = yo`<div></div>`
    self._view.config.plugin = yo`
      <div class="${css.info}">
        <div class=${css.title}>嵌入模块 <i title="This section is still under heavy development, please use it carefully" class="${css.icon} fa fa-exclamation-triangle" aria-hidden="true"></i> </div>
        <div class="${css.crowNoFlex}">
          <div>使用JSON格式导入嵌入模块： </div>
          ${self._view.pluginInput}
          <input onclick=${onloadPluginJson} type="button" value="导入" class="${css.initPlugin}">
