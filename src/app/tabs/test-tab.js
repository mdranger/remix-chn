    ...
    var el = yo`
      <div class="${css.testTabView}" id="testView">
        ...
        <div class="${css.title}">单元测试</div>
          这里使用了一个示例来展示如何对智能合约进行单元测试 (打开 ballot_test.sol 文件来查看示例).
          <br/>
          更多信息可以在说明页面找到 <a href="https://remix.readthedocs.io/en/latest/unittesting_tab.html">documentation</a>
          合约的单元测试也可以使用 NPM 组件 remix-tests 
          <a href="https://www.npmjs.com/package/remix-tests">https://www.npmjs.com/package/remix-tests</a>.
          <br/>
          如想获取更详细的信息，请查看文档中的智能合约测试指南：
          <div class=${css.generateTestFile} onclick=${generateTestFile}>产生合约示例</div>
        </div>
        <div class="${css.tests}">
          ${self.testList}
          <div class=${css.buttons}>
            <div class=${css.runButton} onclick=${runTests}>执行测试</div>
          </div>
          ${testsOutput}
          ${testsSummary}
        </div>
