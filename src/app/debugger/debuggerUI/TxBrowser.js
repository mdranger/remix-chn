  ...
  var view = yo`<div class="${css.container}">
        <div class="${css.txContainer}">
          ...
          <div class="${css.txbuttons}">
            <button id='load' class='${css.txbutton}' title='start debugging' onclick=${function () { self.submit() }}>开始调试</button>
            <button id='unload' class='${css.txbutton}' title='stop debugging' onclick=${function () { self.unload() }}>停止</button>
          </div>
        </div>
        <span id='error'></span>
      </div>`
