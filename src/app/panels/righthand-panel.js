...
module.exports = class RighthandPanel {
  constructor (localRegistry) {
    ...
    const { compile, run, settings, analysis, debug, test } = self._components
    self._components.tabbedMenu.addTab('编译', 'compileView', compile.render())
    self._components.tabbedMenu.addTab('运行', 'runView', run.render())
    self._components.tabbedMenu.addTab('解析', 'staticanalysisView', analysis.render())
    self._components.tabbedMenu.addTab('测试', 'testView', test.render())
    self._components.tabbedMenu.addTab('调试', 'debugView', debug.render())
    self._components.tabbedMenu.addTab('设置', 'settingsView', settings.render())
    // self._components.tabbedMenu.addTab('帮助', 'supportView', support.render())// removed, not using
