...
class Terminal {
  ...
  render () {
    ...
    var intro = yo`<div><div> - 欢迎使用 Remix v0.7.7 - </div><br>
                  <div>本终端界面可以用来: </div>
                  <ul class=${css2.ul}>
                    <li>查看交易详细信息并进行调试。</li>
                    <li>执行 JavaScript 脚本。以下软件库可以使用：
                      <ul class=${css2.ul}>
                    ...
                    <li>可以执行与Remix界面通讯的一些常用命令 (常用命令见下方信息，也可在命令行输入remix.help()进行展示)。这些命令也可以在 JavaScript 脚本文件中使用和执行。</li>
                    <li>在不同脚本执行时，可以使用 exports/.register(key, obj)/.remove(key)/.clear() 命令来注册和重复使用 object。</li>
                  </ul>
                  </div>`
